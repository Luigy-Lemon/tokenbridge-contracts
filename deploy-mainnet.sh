#!/usr/bin/env bash
source .env
JSON_OUTPUT=$(forge create --rpc-url=$RPC_MAINNET --mnemonic="$MNEMONIC" --json --optimize XDaiForeignBridge)

DEPLOYED_TO=$(jq -r '.deployedTo' <<< "$JSON_OUTPUT")

forge verify-contract --watch --chain mainnet $DEPLOYED_TO XDaiForeignBridge
